...
Re-renders the module's template-managed files from the latest templates and
overwrites them, so a template change can be rolled out across many modules
without hand-editing each one.
```
jig renew [--dry-run]
```

Only files matching the `[renew]` paths allowlist in the module's `jig.toml`
are touched. The allowlist is empty by default, so nothing can be overwritten
until the module opts in. Patterns are gitignore-style globs matched against
...
the module does not have yet (added to the template tree after scaffolding)
are created. `metadata.json` and `jig.toml` are generated by jig and are
never renewed. With `--dry-run` nothing is written; each file that would
change is reported with a diff.

The template source is resolved the same way as for `jig new` (it accepts
the same `--template-dir`, `--template-url`, `--template-ref`, and
...
`template_dir` from the config. After a successful renew from a remote
repository the `[template]` section of `jig.toml` is updated to the commit
that was fetched. Note that this rewrites `jig.toml`, so hand-written
...
# Files `jig renew` may re-render and overwrite. Empty by default so nothing
# is overwritten accidentally. See "jig renew" above.
[renew]
paths = []
